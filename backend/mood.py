...
async def get_week_moods(user_id: str):
    ...
    stats = {
        "weekly_average": round(total_score / total_entries, 1) if total_entries > 0 else 0,
        "total_entries": total_entries,
        "best_day": max(week_data, key=lambda x: x["average_score"])["date"] if week_data else "N/A",
        "worst_day": min(week_data, key=lambda x: x["average_score"])["date"] if week_data else "N/A"
    }
